...
function setTokens(images, options, css) {
  return new Promise(function (resolve) {
    css.walkAtRules('lazysprite', function (atRule) {
      ...
      var mediaAtRule2x = postcss.atRule({
        name: 'media',
        params: resolutions2x.join(', ')
      });
      var mediaAtRule3x = postcss.atRule({
        name: 'media',
        params: resolutions3x.join(', ')
      });
      ...
      const usePseudoBefore = options.usePseudoBefore;
      ...
      _.forEach(images, function (image) {
        ...
        if (sliceDirname === image.dir) {
          ...
          var selector = '.' + options.nameSpace + image.selector;
          // Add `source` argument for source map create.
          var singleRule = postcss.rule({
            selector: selector + (usePseudoBefore ? ':before' : ''),
            source: atRule.source
          });
          ...
          if (usePseudoBefore) {
            ...
            atRuleParent.insertBefore(atRule, rule);
          }
      ...
      .map(function (images, temp) {
        ...
        if (temp.indexOf('GROUP_SVG_FLAG') > -1) {
          ...
          _.forEach(images, function (item) {
            spriter.add(item.path, null, fs.readFileSync(item.path, {
              encoding: 'utf-8'
            }));
          });
    ...
    css.walkComments(function (comment) {
      ...
      if (isToken(comment)) {
        ...
        if (image) {
          ...
          if (options.usePseudoBefore) {
            backgroundImage.before(postcss.decl({
              prop: 'content',
              value: '\'\''
            }));
            backgroundImage.before(postcss.decl({
